...
public:
	void addNewBooking(vector<Customer*> customers, vector<Instrument*>instruments)
	{
		...
		cout << "Are you going to buy the instrument after the booking period?: Y/N" << endl;
			...
			if (choice == "Y" || choice == "y")
			{
				...
				QuotedPrice += accessories[choice]->getaccessoryPrice() * quantity;
			}
	...
	{
		//output <<instrumentchoice<<endl<<tempinstrument->getinstrumenttype()<<endl<< tempinstrument->getinstrumentname() << endl << tempinstrument->getinstrumentmake() << endl << tempinstrument->getinstrumentmodel() << endl << tempinstrument->getinstrumentnotes() << endl  << tempinstrument->getinstrumentPrice() << endl;
		output << instrumentchoice << endl;
	};

	void writecustomerToFile(ofstream& output)
	{
		//output <<customerchoice<<endl<< tempcustomer->getname() << endl << tempcustomer->getaddress() << endl << tempcustomer->getPhonenumber() << endl << tempcustomer->getNotes()<<endl;
		output << customerchoice<<endl;
	}

	void writetheresttofile(ofstream& output)
	{
		output << lessonRequired << endl << Date << endl << BookingBlock << endl << QuotedPrice << endl;
	}
...
public:
	...
	int getnumberofstrings(int i, vector<Instrument*>Potato)
	{
...
public:
	...
	void createBooking()
	{
		...
		b->addNewBooking(customers, instruments);
	...
	void searchInstrumentName()
	{
		...
		for (unsigned int i = 0; i < instruments.size(); i++)
		{
			if (name == instruments[i]->getinstrumentname())
			{
	...
	void searchAccessoryName()
	{
		...
		for (unsigned int i = 0; i < accessories.size(); i++)
		{
			if (name == accessories[i]->getaccessoryName())
			{
